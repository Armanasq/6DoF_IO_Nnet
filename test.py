...
def main():
    ...
    model = load_model(args.model, custom_objects={'TCN':TCN})
    ...
    if args.dataset == 'oxiod':
        ...
        gyro_data, acc_data, pos_data, ori_data = load_broad_dataset(args.input, args.gt)
    ...
    if args.dataset == 'oxiod':
        [yhat_delta_p, yhat_delta_q] = model.predict([x_gyro[0:200, :, :], x_acc[0:200, :, :]], batch_size=100, verbose=1)
    elif args.dataset == 'euroc':
        [yhat_delta_p, yhat_delta_q] = model.predict([x_gyro, x_acc], batch_size=1, verbose=1)
    elif args.dataset == 'broad':
        [yhat_delta_p, yhat_delta_q] = model.predict([x_gyro, x_acc], batch_size=4000, verbose=1)
