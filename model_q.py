...
from keras.layers import Bidirectional, LSTM, CuDNNLSTM, Dropout, Dense, Input, Layer, Conv1D, MaxPooling1D, concatenate, GRU, CuDNNGRU
...
def quat_mult_error(y_true, y_pred):
	...
	w0,x0,y0,z0 = tf.split((tf.multiply(y_pred,[1., -1, -1 , -1]),), num_or_size_splits=4, axis=-1)
	...
	return tf.abs(tf.multiply(2.0, tf.concat(values=[x, y, z], axis=-1)))#(tf.abs(1-tf.sqrt(w*w + z*z )))
...
#############################
# Models: TCN_CuDNNLSTM, TCN_LSTM, TCN_GRU, TCN_CuDNNGRU, CNN_GRU, CNN_CuDNNGRU, CNN_LSTM, CNN_CuDNNLSTM
#TCN_CuDNNLSTM
def create_pred_model_quat_TCN_CuDNNLSTM(window_size=200):
    ...
    convA1 = TCN(128,11,return_sequences=True)(x1)
    convA2 = TCN(128,11,return_sequences=True)(convA1)
    #convA3 = TCN(128,11,return_sequences=True)(convA2)
    poolA = MaxPooling1D(3)(convA2)
    convB1 = TCN(128,11,return_sequences=True)(x2)
    convB2 = TCN(128,11,return_sequences=True)(convB1)
    #convB3 = TCN(128,11,return_sequences=True)(convB2)
    ...
    y1_pred = Dense(4)(drop2)

    model = Model(inputs =[x1, x2], outputs = [y1_pred])
    model.compile(optimizer = keras.optimizers.Adam(), loss =quat_mult_error )
    model.summary()
    
    return model
# TCN_LSTM
def create_pred_model_quat_TCN_LSTM(window_size=200):
    #inp = Input((window_size, 6), name='inp')
    #lstm1 = Bidirectional(CuDNNLSTM(128, return_sequences=True))(inp)
    x1 = Input((window_size, 3), name='x1')
    x2 = Input((window_size, 3), name='x2')
    ...
    poolB = MaxPooling1D(3)(convB2)
    AB = concatenate([poolA, poolB])
    lstm1 = Bidirectional(CuDNNLSTM(32, return_sequences=True))(AB)
    drop1 = Dropout(0.25)(lstm1)
    lstm2 = Bidirectional(CuDNNLSTM(32))(drop1)
    drop2 = Dropout(0.25)(lstm2)    
    y1_pred = Dense(4)(drop2)
    ...
    return model
# TCN_GRU
def create_pred_model_quat_TCN_GRU(window_size=200):
    #inp = Input((window_size, 6), name='inp')
    #lstm1 = Bidirectional(CuDNNLSTM(128, return_sequences=True))(inp)
    x1 = Input((window_size, 3), name='x1')
    x2 = Input((window_size, 3), name='x2')
    ...
    poolB = MaxPooling1D(3)(convB2)
    AB = concatenate([poolA, poolB])
    lstm1 = Bidirectional(GRU(128, return_sequences=True))(AB)
    drop1 = Dropout(0.25)(lstm1)
    lstm2 = Bidirectional(GRU(128))(drop1)
    drop2 = Dropout(0.25)(lstm2)    
    y1_pred = Dense(4)(drop2)
    ...
    return model
# TCN_CuDNNGRU
def create_pred_model_quat_TCN_CuDNNGRU(window_size=200):
    #inp = Input((window_size, 6), name='inp')
    #lstm1 = Bidirectional(CuDNNLSTM(128, return_sequences=True))(inp)
    x1 = Input((window_size, 3), name='x1')
    x2 = Input((window_size, 3), name='x2')
    ...
    poolB = MaxPooling1D(3)(convB2)
    AB = concatenate([poolA, poolB])
    lstm1 = Bidirectional(CuDNNGRU(128, return_sequences=True))(AB)
    drop1 = Dropout(0.25)(lstm1)
    lstm2 = Bidirectional(CuDNNGRU(128))(drop1)
    drop2 = Dropout(0.25)(lstm2)    
    y1_pred = Dense(4)(drop2)
    ...
    return model
# CNN_GRU
def create_pred_model_quat_CNN_GRU(window_size=200):
    #inp = Input((window_size, 6), name='inp')
    #lstm1 = Bidirectional(CuDNNLSTM(128, return_sequences=True))(inp)
    x1 = Input((window_size, 3), name='x1')
    x2 = Input((window_size, 3), name='x2')
    ...
    poolB = MaxPooling1D(3)(convB2)
    AB = concatenate([poolA, poolB])
    lstm1 = Bidirectional(GRU(32, return_sequences=True))(AB)
    drop1 = Dropout(0.25)(lstm1)
    lstm2 = Bidirectional(GRU(32))(drop1)
    drop2 = Dropout(0.25)(lstm2)    
    y1_pred  = Dense(4)(drop2)
    ...
    return model
# CNN_CuDNNGRU
def create_pred_model_quat_CNN_CuDNNGRU(window_size=200):
    #inp = Input((window_size, 6), name='inp')
    #lstm1 = Bidirectional(CuDNNLSTM(128, return_sequences=True))(inp)
    x1 = Input((window_size, 3), name='x1')
    x2 = Input((window_size, 3), name='x2')
    ...
    poolB = MaxPooling1D(3)(convB2)
    AB = concatenate([poolA, poolB])
    lstm1 = Bidirectional(CuDNNGRU(32, return_sequences=True))(AB)
    drop1 = Dropout(0.25)(lstm1)
    lstm2 = Bidirectional(CuDNNGRU(32))(drop1)
    drop2 = Dropout(0.25)(lstm2)    
    y1_pred = Dense(4)(drop2)
    ...
    return model
# CNN_LSTM
def create_pred_model_quat_CNN_LSTM(window_size=200):
    #inp = Input((window_size, 6), name='inp')
    #lstm1 = Bidirectional(CuDNNLSTM(128, return_sequences=True))(inp)
    x1 = Input((window_size, 3), name='x1')
    x2 = Input((window_size, 3), name='x2')
    ...
    poolB = MaxPooling1D(3)(convB2)
    AB = concatenate([poolA, poolB])
    lstm1 = Bidirectional(LSTM(32, return_sequences=True))(AB)
    drop1 = Dropout(0.25)(lstm1)
    lstm2 = Bidirectional(LSTM(32))(drop1)
    drop2 = Dropout(0.25)(lstm2)    
    y1_pred  = Dense(4)(drop2)
    ...
    return model
# CNN_CuDNNLSTM
def create_pred_model_quat_CNN_CuDNNLSTM(window_size=200):
    #inp = Input((window_size, 6), name='inp')
    #lstm1 = Bidirectional(CuDNNLSTM(128, return_sequences=True))(inp)
    x1 = Input((window_size, 3), name='x1')
    x2 = Input((window_size, 3), name='x2')
    ...
    return model
# CNN_TCN
def create_pred_model_quat_CNN_TCN(window_size=200):
    #inp = Input((window_size, 6), name='inp')
    #lstm1 = Bidirectional(CuDNNLSTM(128, return_sequences=True))(inp)
    x1 = Input((window_size, 3), name='x1')
    x2 = Input((window_size, 3), name='x2')
    convA1 = Conv1D(128,1)(x1)
    convA2 = Conv1D(128,1)(convA1)
    #convA3 = TCN(128,11,return_sequences=True)(convA2)
    poolA = MaxPooling1D(3)(convA2)
    convB1 = Conv1D(128,1)(x2)
    convB2 = Conv1D(128,1)(convB1)
    #convB3 = TCN(128,11,return_sequences=True)(convB2)
    poolB = MaxPooling1D(3)(convB2)
    AB = concatenate([poolA, poolB])
    TCN1 = TCN(128,11,return_sequences=True)(AB)
    drop1 = Dropout(0.25)(TCN1)
    TCN2 = TCN(128,11,return_sequences=False)(drop1)
    drop2 = Dropout(0.25)(TCN2)    
    y1_pred = Dense(4)(drop2)
